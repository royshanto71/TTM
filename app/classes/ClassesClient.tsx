...
export default function ClassesClient({ initialClasses, initialStudents }: ClassesClientProps) {
  const [classes, setClasses] = useState<Class[]>(initialClasses);
  const [students, setStudents] = useState<Student[]>(initialStudents);
  const [filteredClasses, setFilteredClasses] = useState<Class[]>(initialClasses);
  ...
  useEffect(() => {
    // Filter classes based on search query and selectedDate
  ...
  async function handleAddClass(e: React.FormEvent) {
    ...
    const newClass: Class = {
      ...
      date: formData.date,
      time: formData.time,
      completed_count: formData.completed_count,
      created_at: new Date().toISOString()
    };
    
    // Update local state immediately
    setClasses(prev => [newClass, ...prev].sort((a, b) => b.date.localeCompare(a.date)));
    ...
    try {
      // Remove temporary ID before sending to DB if needed, or let DB handle it. 
      // Supabase insert returns the actual data.
      const { data, error } = await supabase.from('classes').insert([{
        student_id: newClass.student_id,
        date: newClass.date,
        time: newClass.time,
        completed_count: newClass.completed_count
      }]).select().single();

      if (error) throw error;

      // Replace optimistic item with actual item from DB (to get correct ID)
      setClasses(prev => prev.map(c => c.id === newClass.id ? data : c));
